  ...
  return useSyncExternalStore(
    (onChange) => {
      ...
      const handleSetGlobal = (event: SetGlobalsEvent) => {
        ...
        if (!window.openai) {
          window.openai = {} as any;
        }

        // Merge all globals from the event into window.openai
        // This ensures all hooks get their values, not just this one
        Object.assign(window.openai, event.detail.globals);

        // Notify React to re-read the snapshot
        onChange();
      };
